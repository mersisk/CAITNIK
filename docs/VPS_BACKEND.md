...
Telegram-запрос выполняется через стандартный `node:https`: `SocksProxyAgent`
передаётся в параметре `agent`. Не используйте его как `dispatcher` для
стандартного `fetch` — эти интерфейсы несовместимы. Соединение ограничено
таймаутом 5 секунд; журнал получает только безопасные `message` и `code` без
токена и реквизитов прокси.
